...
import { FaArrowLeft, FaSignOutAlt, FaMountain, FaHome, FaStar, FaMapMarkerAlt, FaRuler, FaClock, FaArrowUp, FaArrowDown } from 'react-icons/fa';
...
export default function FavouritesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/user/basicInfo', { method: 'GET', headers }).then(r => r.json()),
      ...
      .then(([userData, structuresData, trailsData]) => {
        setUser(userData);
        const structures = (structuresData.fav_structures ?? []).map((s: any) => ({ ...s, type: 'structure' }));
        const trails = (trailsData.fav_trails ?? []).map((t: any) => ({ ...t, type: 'trail' }));
  ...
  const filtered = favourites.filter(f => filter === 'all' || f.type === filter);
  const structureCount = favourites.filter(f => f.type === 'structure').length;
  ...
  return (
    <Layout navChildren={
      <UserDropdown
        ...
        items={[
          { label: 'Back', icon: <FaArrowLeft size={14} />, onClick: () => navigate(-1) },
        ]}
      />
    }>
      <div className="fav-page">
        ...
        <div className="fav-header">
          <div className="fav-header-left">
            ...
            <p className="fav-subtitle">All your favourite trails and structures in one place.</p>
          </div>
        </div>
            ...
            {filtered.map(item => (
              <div key={item._id} className="fav-card">
                ...
                <div className="fav-card-top">
                  <span className={`fav-type-badge fav-type-badge--${item.type}`}>
                    {item.type === 'trail' ? <FaMountain size={11} /> : <FaHome size={11} />}
                ...
                <div className="fav-card-info">
                  {item.coordinates && (
                    <div className="fav-info-row">
                      ...
                      <span>
                        ...
                        {item.coordinates.altitude ? ` · ${item.coordinates.altitude}m` : ''}
                      </span>
                    </div>
                  )}
                  {item.telephone && (
                  ...
                  {item.difficulty && (
                    <div className="fav-info-row">
                      ...
                      <span style={{ color: difficultyColor(item.difficulty), fontWeight: 600 }}>
                        {item.difficulty}
                      </span>
                    </div>
                  )}
                  ...
                  {item.ascent_m && (
                    <div className="fav-info-row">
                      ...
                      {item.descent_m && (
                        <>
                          <FaArrowDown size={12} className="fav-info-icon" style={{ marginLeft: 8 }} />
